...
export class Toast extends LitElement {
    ...
    render() {
        return html`<div class="fixed w-full ${this.bottom ? "bottom-20" : "top-0"} left-0 flex items-center justify-center z-30">
            <div
                id="box"
                class="animate-fade animate-duration-[500ms] w-full max-w-[300px] p-4 flex justify-center items-center bg-black text-white fill-white rounded-md fancy-shadow"
            >
                ${this.content}
            </div>
        </div>`;
...
export function toast(content: TemplateResult | HTMLElement | string, timeout = 2500) {
